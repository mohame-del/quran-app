                            ...
                            {showSchool && (
                                <div className="absolute top-full mt-2 right-0 w-72 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg z-50 p-4">
                                    <div className="text-right">
                                        ...
                                        <div className="font-semibold text-gray-900 dark:text-white">{user?.schoolName || '—'}</div>
                                        <div className="text-xs text-gray-400 mt-1">المدرسين: {user?.name || '—'}</div>
                                        <div className="text-xs text-gray-400 mt-1">عدد الطلبة: {students.length}</div>
                                    </div>

                                    <div className="mt-3 grid grid-cols-1 gap-2">
                                        <button onClick={() => { setShowSchool(false); router.push('/my-school') }} className="w-full text-right px-3 py-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-200">عرض تفاصيل المدرسة</button>
                                        <button onClick={() => { setShowSchool(false); router.push('/schools/new') }} className="w-full text-right px-3 py-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-200">إنشاء مدرسة جديدة</button>
                                        <button onClick={() => { setShowSchool(false); router.push('/students') }} className="w-full text-right px-3 py-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-200">تبديل المدرسة</button>
                                    </div>
                                </div>
                            )}
                        </div>
